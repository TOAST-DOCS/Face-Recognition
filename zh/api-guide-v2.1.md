## AI Service > Face Recognition > API v2.1 Guide
...
* This API creates groups. You can use [Register Face](./api-guide-v2.1/#add-face) to a created group to register faces.
...
```shell
$ curl -X POST '{domain}/v2.1/appkeys/{appKey}/groups' -H 'Authorization: {secretKey}' -H 'Content-Type: application/json;charset=UTF-8' -d '{
    "groupId": "my-group"
}'
```
...
* [Response body header description omitted]
    * This information is available in [Common Response Information](./api-guide-v2.1/#common-response)
...
* The saved vector data gets used as characteristic vectors for the [Search face by face ID](./api-guide-v2.1/#search-by-face-id)and [Search face by image](./api-guide-v2.1/#search-by-image) APIs.
* The input image can be delivered via Base64-encoded image bytes or image url.
* To find out more about input image, see [Input Image Guide](./api-guide-v2.1/#input-image-guide).
...
* 'imageId' and 'externalImageId' are returned from the response of the [Face list within a group](./api-guide-v2.1/#face-list-in-a-group) and [Search face by face ID](./api-guide-v2.1/#search-by-face-id) and [Search face by image](./api-guide-v2.1/#search-by-image) APIs.
...
```shell
$ curl -X POST -H 'x-nhn-authorization: Bearer {User Access Key Token}' -H 'Content-Type: multipart/form-data' -F imageFile=@image.png -F externalImageId=image01.jsp -F limit=3 '{domain}/v2.1/appkeys/{appKey}/groups/{groupId}/faces'
```
...
```shell
$ curl -X GET '{domain}/v2.1/appkeys/{appKey}/groups/{groupId}/faces?limit={limit}&next-token={next-token}' -H 'x-nhn-authorization: Bearer {User Access Key Token}' -H 'Content-Type: application/json;charset=UTF-8'
```
...
```shell
$ curl -X POST '{domain}/v2.1/appkeys/{appKey}/groups/{groupId}/faces/search' -H 'x-nhn-authorization: Bearer {User Access Key Token}' -H 'Content-Type: application/json;charset=UTF-8' -d '{
    "image": {
        "url": "https://..."
    },
    "limit": 100
...
```shell
$ curl -X POST '{domain}/v2.1/appkeys/{appKey}/faces/compare' -H 'x-nhn-authorization: Bearer {User Access Key Token}' -H 'Content-Type: application/json;charset=UTF-8' -d '{
    "sourceImage": {
        "url": "https://..."
    },
    "targetImage": {
...
* Use [Register Face](./api-guide-v2.1/#add-face) to a created group to register faces.
